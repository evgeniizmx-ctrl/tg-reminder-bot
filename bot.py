...
# !!! расширили: допускаем «в 17 часов» внутри «сегодня/завтра/послезавтра»
...
def parse_dayword_time(text: str):
    ...
    # если явно "час/часов" — считаем однозначным
    if re.search(r"\bчас(ов|а)?\b", m.group(0), re.I):
...
async def cmd_start(m: Message):
    await m.answer(
        ...
        "«завтра в 6», «в 17 часов», «в это же время завтра», «в 7».\n"
...
async def on_text(m: Message):
    ...
    # 6) «в HH[:MM]» — двусмысленно (последним)
    ...
    await m.reply("Не понял дату/время. Примеры: «24.05 19:00», «24 мая в 19», «1 числа в 7», «через 30 минут», «завтра в 6», «в 17 часов».")
